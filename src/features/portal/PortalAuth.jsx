...
export default function PortalAuth({ supabase, onAuthenticated, user, mode: initialMode }) {
  const [mode, setMode] = useState(initialMode || 'login'); // login | signup | forgot | updatePassword
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      showError(err.message || 'Failed to update password. The link may have expired.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <form onSubmit={mode === 'signup' ? handleSignUp : handleSignIn}>
        ...
        {mode === 'login' && (
          <div style={{ textAlign: 'center', marginBottom: 12 }}>
            <a onClick={() => setMode('forgot')} style={{ fontSize: '0.82rem', color: 'var(--accent)', cursor: 'pointer' }}>
              Forgot password?
            </a>
          </div>
        )}
