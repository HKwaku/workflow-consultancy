 ...
 *
 * `potential_savings` is intentionally always 0 here. Savings only exist
 * once a change to the process has been DECIDED — that lives in the
 * `changes` table, not in flow_data, and this function is a pure sync
 * derivation over flow_data. Callers that can reach the DB enrich
 * potential_savings from accepted changes via
 * lib/changes/savings.js#decidedSavingsFromChanges. We no longer surface
 * the old speculative "if you automated everything" heuristic.
 */
export function deriveProcessMetrics(rowOrFlowData) {
  ...
  return {
    ...
    potential_savings:     0, // decided-changes only — enriched by callers
