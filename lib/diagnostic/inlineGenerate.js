...
export async function generateReportInline(input, deps) {
  ...
  const reportPayload = {
    ...
    // Direct deal scope for signed-in users mapping inside an existing
    // deal. Server matches caller's email against deal_participants on
    // this deal to auto-mark the participant complete on report save.
    dealId: dealId || null,
  };
