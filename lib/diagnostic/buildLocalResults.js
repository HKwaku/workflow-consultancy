...
export function generateRuleBasedRecommendations(processes) {
  ...
  // Stable id + default review state so reviewer PATCH endpoint can target each rec.
  const stamp = Date.now().toString(36);
  return recs.map((r, i) => ({ id: r.id || `rec_${stamp}_rb${i}`, reviewStatus: r.reviewStatus || 'pending', ...r }));
}

/**
 * Build diagnostic results client-side (fallback path retained for inline
 * fallbacks that used to compare against the now-removed /api/process-diagnostic).
