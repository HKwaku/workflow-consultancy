...
export const INTRO_PROMPTS = [
  {
    id: 'path',
    question: "Hello, I'm Reina! I'll help you map your process and find where time and money are leaking. What would you like to do?",
    chips: [
...
export const COMPREHENSIVE_PROMPTS = [
  ...
  {
    id: 'segment',
    /** Skip when segment was already chosen on the audit gate */
    shouldShow: (data) => !data?.segment,
...
/**
 * Next index after answering `fromIndex`, skipping prompts whose `shouldShow(merged)` is false.
 * Matches IntroChatScreen / GuidedChatScreen advance behaviour.
 */
export function nextPromptIndexAfter(fromIndex, merged, prompts) {
  let i = fromIndex + 1;
  while (i < prompts.length && prompts[i].shouldShow && !prompts[i].shouldShow(merged)) {
    i += 1;
  }
  return i;
}
