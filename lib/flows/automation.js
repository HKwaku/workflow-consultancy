...
export function classifyAutomation(step, stepIdx, process) {
  const result = _classifyStep(step, stepIdx, process);
  if (!step.isExternal) return result;

  // External steps can't be simple automation - they need third-party integration at minimum.
  // Null or simple → floor to agent. Agent/human-loop/multi-agent → keep category, add note.
  if (!result || result.key === 'simple') {
    return { ...AUTOMATION_CATEGORIES.agent, reason: 'External dependency - automate handoff tracking, chasing, and confirmation; the external action itself remains outside your control.' };
  }
  return { ...result, reason: result.reason + ' External dependency - requires third-party integration or API.' };
}

function _classifyStep(step, stepIdx, process) {
