...
export function prepareSteps(process) {
  ...
  // The Start node ALWAYS reads "Start". Don't pull the trigger
  // (process.definition.startsWhen) into the label — users have
  // repeatedly reported that putting trigger content inside the Start
  // node makes it look like a step. The trigger lives in the process
  // boundary metadata (Screen 1, set_process_definition); the Start
  // node is the abstract entry marker only.
  const startLabel = 'Start';
  const endLabel = 'Complete';
  ...
  const allSteps = steps.map((s, i) => {
    ...
    return {
      ...
      // Pass-through workspace anchors so swimlane lane-grouping can
      // resolve sub-function / function names. The prepared step is what
      // processToReactFlow sees; without these, laneLabelForStep can only
      // fall back to the role label and the toggle is a no-op.
