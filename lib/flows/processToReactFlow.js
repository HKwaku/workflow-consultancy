...
export function processToReactFlow(process, layout = 'grid', darkTheme = false, opts = {}) {
  ...
  if (isWrap) {
    ...
    allSteps.forEach((decStep, d) => {
      // Parallel and inclusive decisions need their branch terminals auto-wired to a merge.
      // Exclusive decisions take exactly one branch so they never converge via a merge node.
      if (!decStep.isDecision || (!decStep.parallel && !decStep.inclusive) || !decStep.branches?.length) return;
