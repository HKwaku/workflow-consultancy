...
/** Build a colour map for all unique lane keys in appearance order. When
 *  `keyOf` is null the map is keyed on `step.department` (grid layouts);
 *  swimlane layouts pass a resolver that returns the active lane label
 *  (role / sub-function / function name). */
function buildDeptColorMap(allSteps, keyOf = null) {
  const resolve = keyOf || ((s) => s.department);
  const map = {};
  let idx = 0;
  allSteps.forEach((s) => {
    const key = resolve(s);
    if (key != null && !(key in map)) {
      map[key] = { ...DEPT_PALETTE[idx % DEPT_PALETTE.length], name: key };
      idx++;
    }
  });
  return map;
...
/**
 * Resolve the swimlane-lane label for one step. The swimlane view supports
 * three groupings; the caller picks one with `mode`:
 *   - 'role'        (default) — step.department (role label)
 *   - 'subfunction'           — leaf-function name (resolves step.functionId,
 *                                falls back to role.function_ids[0])
 *   - 'function'              — top-level parent of the leaf function
 *
 * `fnIndex` is a Map<id, fn> built from functionsFlat; `roleIndex` is a
 * Map<id, role> built from the roles list. Both are optional — if either is
 * missing we degrade gracefully to the role label so the chart never breaks.
 */
function laneLabelForStep(step, mode, fnIndex, roleIndex) {
  const roleLabel = (step.department || '').trim() || 'Other';
  if (!mode || mode === 'role') return roleLabel;

  // Resolve the step's leaf function: prefer step.functionId, else the
  // first function on its role (roleId → role.function_ids[0]).
  let leafFnId = step.functionId || step.function_id || step.capabilityId || step.capability_id || null;
  if (!leafFnId && step.roleId && roleIndex) {
    const role = roleIndex.get(step.roleId);
    const fids = role && (role.function_ids || role.capability_ids);
    if (Array.isArray(fids) && fids[0]) leafFnId = fids[0];
  }

  if (!leafFnId || !fnIndex) return roleLabel;
  const leaf = fnIndex.get(leafFnId);
  if (!leaf) return roleLabel;

  if (mode === 'subfunction') return leaf.name || roleLabel;

  // mode === 'function' → walk up to the top-level parent
  let cursor = leaf;
  while (cursor && (cursor.parent_function_id || cursor.parent_capability_id)) {
    const parentId = cursor.parent_function_id || cursor.parent_capability_id;
    const parent = fnIndex.get(parentId);
    if (!parent) break;
    cursor = parent;
  }
  return cursor?.name || leaf.name || roleLabel;
}

/**
 * Compute swimlane positions (horizontal flow per lane). The lane key is
 * pre-computed by the caller via `laneKeyOf(step)` so the same layout code
 * supports role / sub-function / function groupings.
 */
function computeSwimlaneLayout(allSteps, laneKeyOf = (s) => s.department) {
  const deptOrder = [];
  const deptMap = {};
  allSteps.forEach((s) => {
    const key = laneKeyOf(s) || 'Other';
    if (!deptMap[key]) {
      deptMap[key] = [];
      deptOrder.push(key);
    }
  ...
  const firstLane = lanes.find((l) => l.dept === laneKeyOf(allSteps[0]));
  const lastLane = lanes.find((l) => l.dept === laneKeyOf(allSteps[allSteps.length - 1]));
...
function fanDecisionBranches(nodes, allSteps) {
  ...
  allSteps.forEach((s, i) => {
    ...
    // Reposition merge node: right of all branch tails, centred on decision Y.
    // Only the merge that actually belongs to this decision - stop searching if
    // another decision appears first, since that merge belongs to it.
    let mergeStep = null;
    let mi = -1;
    for (let k = i + 1; k < allSteps.length; k++) {
      if (allSteps[k].isDecision && (allSteps[k].branches || []).length) break;
      if (allSteps[k].isMerge) { mergeStep = allSteps[k]; mi = k; break; }
    }
    if (mergeStep) {
...
export function processToReactFlow(process, layout = 'grid', darkTheme = false, opts = {}) {
  ...
  // Falls back to 'role' so older callers keep their current behaviour.
  const swimlaneBy = opts.swimlaneBy || 'role';
  const fnIndex   = opts.functionsFlat ? new Map(opts.functionsFlat.map((f) => [f.id, f])) : null;
  const roleIndex = opts.roles         ? new Map(opts.roles.map((r) => [r.id, r]))         : null;
  const laneKeyOf = (s) => laneLabelForStep(s, swimlaneBy, fnIndex, roleIndex);
  // Stamp the resolved lane label onto each step in a private field so
  // downstream colour + label code can read the same key the layout used.
  // Avoids re-resolving in two places.
  if (!isGrid) {
    allSteps.forEach((s) => { s.__laneLabel = laneKeyOf(s); });
  }
  const deptColorMap = buildDeptColorMap(allSteps, isGrid ? null : ((s) => s.__laneLabel));
  const layoutData = layout === 'grid'
    ? computeLinearLayout(allSteps)
    : layout === 'wrap'
      ? computeWrapLayout(allSteps, effectiveMaxCols)
      : computeSwimlaneLayout(allSteps, (s) => s.__laneLabel);
  ...
  allSteps.forEach((s, i) => {
    ...
    const isDecision = s.isDecision && (s.branches || []).length > 0;
    // In swimlane mode, colour follows the active lane key (role / sub /
    // function); in grid mode it follows the role label as before.
    const colorKey = !isGrid ? s.__laneLabel : s.department;
    const deptEntry = colorKey != null ? deptColorMap[colorKey] : null;
    const dc = deptEntry
      ? {
  ...
  if (isWrap) {
    ...
    allSteps.forEach((decStep, d) => {
      ...
      // Find the merge node that belongs to this decision: the first isMerge
      // step after the furthest branch target, provided no OTHER decision lies
      // between this decision and that merge. Without the decision-break, a
      // downstream merge intended for a later decision would get wired to
      // every earlier decision's branch terminals.
      const maxTarget = Math.max(...targets);
      let mergeIdx = -1;
      for (let m = maxTarget + 1; m < allSteps.length; m++) {
        if (allSteps[m].isDecision && (allSteps[m].branches || []).length) break;
