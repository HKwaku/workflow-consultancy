...
export async function resolveDealAccess({ dealId, email, userId }) {
  ...
  if (ownerMatch) {
    auditLog({
      action: 'deal.access_resolved',
      actorEmail: email, actorUserId: userId,
      dealId, targetType: 'deal', targetId: dealId,
      details: { mode: 'owner' },
    });
  ...
  if (partRows.length) {
    auditLog({
      ...
      actorEmail: email, actorUserId: userId,
      dealId, targetType: 'deal', targetId: dealId,
      details: { mode: 'participant', participantRole: partRows[0].role },
    });
    return {
      mode: 'participant',
      deal,
