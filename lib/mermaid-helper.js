import { repairFlow } from './flows/normalizer.js';
...
export function generateMermaidCode(processes) {
  let m = 'graph TB\n';

  processes.forEach((rawProcess, pi) => {
    const { steps: repairedSteps } = repairFlow(rawProcess.steps || []);
    const p = { ...rawProcess, steps: repairedSteps };
    ...
    steps.forEach((s, i) => {
      ...
      s.branches.forEach(br => {
        const targetIdx = resolveBranchTarget(br.target, steps);
