...
export const recordRecommendationTool = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      dealStage: z
        .enum(['day1', 'tsa', 'separation', 'post-close'])
        .optional()
        .describe(
          'M&A integration timeline tag — set when the engagement is M&A or PE. day1 = must be in place at close; tsa = covered by transitional services agreement; separation = standalone build needed before separation; post-close = first-100-days improvement. Omit for non-deal contexts.'
        ),
      sources: z
        .array(z.object({
          docId: z.string().max(200).optional().describe('Stable id of the source document if known'),
          docName: z.string().max(300).optional().describe('Filename or title of the source document'),
          locator: z.string().max(200).optional().describe('Where in the doc — e.g. "slide 14", "page 3", "Sheet1!B12:D40", "section 4.2"'),
          quote: z.string().max(500).optional().describe('Short verbatim quote backing the finding (≤500 chars)'),
        }))
        .max(5)
        .optional()
        .describe(
          'Source citations backing this recommendation. Populate when the finding is grounded in a specific uploaded document; otherwise omit.'
        ),
    }),
  }
);
