 ...
 * @param {object} contact - Contact info ({ name, email, company }).
 ...
 */
export async function runRecommendationsAgent(processes, contact, onProgress, requestId) {
  ...
  const segmentBlock = contact?.segment ? `\n${SEGMENT_CONTEXT[contact.segment] || ''}\n` : '';
...
Analyse the data and produce 5-8 ranked recommendations. Make ALL tool calls in a single response.`;
  ...
  const messages = [
    new SystemMessage(RECOMMENDATIONS_SYSTEM),
