...
- For M&A or PE engagements, set dealStage on each recommendation: day1 (must be in place at close), tsa (covered by transitional services), separation (standalone build before separation), or post-close (first-100-days). Omit dealStage for scaling/operational engagements.
- When a finding is directly grounded in an uploaded document (PDF/PPTX/XLSX/DOCX referenced in the data), populate sources with { docName, locator, quote }. locator must be specific: "slide 14", "page 3", "Sheet1!B12:D40", "section 4.2". Skip sources entirely for findings derived from the structured intake.
...
function extractRecommendations(toolCalls) {
  return toolCalls
    ...
    .map((tc, idx) => {
      ...
      return { id: args.id || `rec_${Date.now().toString(36)}_${idx}`, reviewStatus: 'pending', ...args };
    });
}
