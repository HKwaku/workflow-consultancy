...
export async function computeModelIntro({ operatingModelId, operatingModelName }) {
  ...
  // With nothing mapped yet, the one thing that matters is starting a
  // process — say so plainly instead of burying it in a generic list.
  const lines = procCount === 0
    ? [
      summaryParts.join(' '),
      '',
      `**Start by mapping a process.** Nothing gets analysed until there's a process in the model — tell me its name and what kicks it off (e.g. "map our invoice approval, it starts when an invoice arrives") and I'll build the flow live as we talk. You can also paste an SOP or drop a doc and I'll extract it.`,
      '',
      `It gets filed under a function inside **${name}**, so the workspace can roll cost, bottlenecks and automation up across the model.`,
    ]
    : [
      summaryParts.join(' '),
      '',
      'What would you like to do? I can:',
      '- **Map a new process** — just describe it and I build the flow live',
      ...
      '- **Share a process with a colleague** (open it, then use the "Share" link: read-only, no account needed)',
    ];
  return lines.join('\n');
}
...
async function loadDealFindingCount(dealId) {
  ...
  try {
    ...
    const [findResp, procResp] = await Promise.all([
      fetchWithTimeout(
        `${sb.url}/rest/v1/deal_findings?deal_id=eq.${encodeURIComponent(dealId)}&select=id&limit=1`,
        { method: 'GET', headers },
      ),
      fetchWithTimeout(
