...
export const PROPOSE_ADD_SYSTEM_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      layer:       { type: 'string', enum: ['system_of_record', 'productivity', 'workflow', 'analytics', 'comms', 'other'], description: 'Optional architectural layer.' },
      ...
      systems: {
        ...
        items: {
          ...
          properties: {
            ...
            layer:       { type: 'string', enum: ['system_of_record', 'productivity', 'workflow', 'analytics', 'comms', 'other'] },
            owner_email: { type: 'string' },
            description: { type: 'string' },
          },
          required: ['name'],
...
export const PROPOSE_UPDATE_SYSTEM_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      layer:       { type: 'string', enum: ['system_of_record', 'productivity', 'workflow', 'analytics', 'comms', 'other'], description: 'New architectural layer.' },
      owner_email: { type: 'string', description: 'New owner email.' },
      description: { type: 'string', description: 'New one-liner.' },
    },
    required: ['system_id'],
...
export const MODEL_AGENT_TOOLS = [
  ...
  PROPOSE_UPDATE_SYSTEM_TOOL,
  PROPOSE_DELETE_SYSTEM_TOOL,
  // Build a NEW process from an uploaded doc/image or a full spoken
  // description, live in this model. Without these the model agent can
  // see an uploaded process but has no way to act on it and falls back
  // to "I can't ingest files" + manual options. The new process files
  // into the active model on save (operating_model_id is set there).
  REPLACE_ALL_STEPS_TOOL,
  SET_PROCESS_NAME_TOOL,
  SET_PROCESS_DEFINITION_TOOL,
  // Artefact emission (schema-free generated output)
