...
export const PROPOSE_FINDING_REVIEW_TOOL = {
  name: 'propose_finding_review',
  description:
    'Stage a finding-review action (approve, reject, needs_revision) for the user to confirm with one click. Does NOT apply the change directly — the client surfaces an Apply button, the user must click. Use ONLY after listing findings (so the user knows the finding_key) or when the user asks to act on a finding they referenced. Editor-only at apply time (owner/collaborator).',
  input_schema: {
    type: 'object',
    properties: {
      findingKey: { type: 'string', description: 'Stable finding key from list_deal_findings (e.g. "tech.tld_concentration").' },
      status: {
        type: 'string',
        enum: ['approved', 'rejected', 'needs_revision'],
        description: 'Review decision to propose.',
      },
      note: { type: 'string', description: 'Optional reviewer note shown alongside the apply button.' },
    },
    required: ['findingKey', 'status'],
  },
};

/**
 * Per-scope report generation. Stages a picker proposal — the client renders
 * checkboxes for participants and processes so the user can confirm exactly
 * what's included before any work runs.
 *
 * Four scopes:
 *   - process_per_company:        one process · one company → opens that
 *                                 participant's diagnostic report (partial
 *                                 ok; picker flags incomplete steps)
 *   - company_rollup:             all/many processes · one company → opens
 *                                 all of that participant's reports
 *   - process_across_companies:   one process · all/many companies → runs an
 *                                 analyse with mode=comparison filtered to
 *                                 that process across the picked companies
 *   - multi_company_multi_process: many processes · many companies → runs
 *                                 an analyse with the chosen mode
 *                                 (comparison/synergy/redesign)
 *
 * Args are inputs to the picker — the user confirms before anything runs.
 */
export const PROPOSE_GENERATE_REPORT_TOOL = {
  name: 'propose_generate_report',
  description:
    'Stage a deal-scoped report generation. Use when the user asks to "generate a report" or "produce a memo" inside a deal context. Always emits a picker the user must confirm before anything runs. Pick the scope based on the user\'s request:\n' +
    '  - process_per_company: single process for a single company (e.g. "show me Acme\'s hiring report")\n' +
    '  - company_rollup: all processes for one company (e.g. "give me Acme\'s full audit")\n' +
    '  - process_across_companies: a single process across multiple companies (e.g. "compare hiring across Acme and Beta")\n' +
    '  - multi_company_multi_process: many processes across many companies (e.g. "run a synergy analysis on the deal")\n' +
    'Pre-fill participantIds / processNames as best you can from the conversation; the user can adjust in the picker. Editor-only at apply.',
  input_schema: {
    type: 'object',
    properties: {
      scope: {
        type: 'string',
        enum: ['process_per_company', 'company_rollup', 'process_across_companies', 'multi_company_multi_process'],
      },
      participantIds: {
        type: 'array',
        items: { type: 'string' },
        description: 'Participant UUIDs (from list_deal_participants). Omit for all participants.',
      },
      processNames: {
        type: 'array',
        items: { type: 'string' },
        description: 'Process names to include. Omit for all processes the participants have mapped.',
      },
      mode: {
        type: 'string',
        enum: ['standard', 'comparison', 'synergy', 'redesign'],
        description: 'Required for process_across_companies + multi_company_multi_process. Ignored for the other scopes.',
      },
    },
    required: ['scope'],
  },
...
export const PROPOSE_RUN_ANALYSIS_TOOL = {
  name: 'propose_run_analysis',
  description:
    'Stage a request to run a deal analysis. Modes: comparison (cross-participant overlap), synergy (FTE/system overlap), redesign (unified target process), diligence (red flags + cross-cuts from the data room). Does NOT start the analysis — the client surfaces an Apply button. The deal owner/collaborator confirms and the analysis runs async; the response contains the new analysis id.',
  input_schema: {
    type: 'object',
    properties: {
      mode: {
        type: 'string',
        enum: ['comparison', 'synergy', 'redesign', 'diligence'],
        description: 'Analysis mode to run. "diligence" is the only mode that runs without participant maps (uses the data room).',
      },
      reason: { type: 'string', description: 'Optional one-liner shown to the user explaining why you suggested this analysis.' },
    },
    required: ['mode'],
  },
};

export const PROPOSE_EXPORT_PPTX_TOOL = {
  name: 'propose_export_pptx',
  description:
    'Stage an export of the latest completed diligence analysis to PowerPoint (.pptx). Only proposes if the latest diligence analysis has at least one approved finding (the export skips non-approved). Apply triggers the download in the user\'s browser. Editor-only.',
  input_schema: { type: 'object', properties: {} },
};
...
export const PROPOSE_LINK_PARTICIPANT_REPORT_TOOL = {
  ...
  description:
    'Stage linking an existing diagnostic report (the user\'s own work) to a participant slot on the current deal. Use when the user says "use my last audit for Acme" or "link report X to the target slot". Call list_reports first if you need report ids, and list_deal_participants for participant ids. Editor-only at apply.',
  input_schema: {
    type: 'object',
    properties: {
      participantId: { type: 'string', description: 'Participant UUID from list_deal_participants.' },
      reportId: { type: 'string', description: 'Diagnostic report id from list_reports.' },
    },
...
export const PROPOSE_UNDO_LAST_ACTION_TOOL = {
  ...
  description:
    'Stage an undo for a recent reversible deal action. Supported kinds: "finding_review" (resets a review back to pending), "link_participant_report" (unlinks a report from a participant slot). Other actions (analysis runs, exports, invites, uploads, reprocess) are NOT undoable from chat — they have side effects (token spend, sent emails, queued workers). Use when the user says "undo that" / "revert" / "I changed my mind" about an action they recently took.',
  input_schema: {
    type: 'object',
    properties: {
      kind: { type: 'string', enum: ['finding_review', 'link_participant_report'], description: 'Which kind of action to undo.' },
      findingKey: { type: 'string', description: 'Required when kind=finding_review.' },
...
export const ALL_CHAT_TOOLS = [
  ...
  // Deal metadata reads (no data-room search required)
  ...
  PROPOSE_FINDING_REVIEW_TOOL,
  PROPOSE_RUN_ANALYSIS_TOOL,
  PROPOSE_EXPORT_PPTX_TOOL,
