...
// Pattern bank for "this is a trigger event, not a step". The agent
// keeps relapsing on certain phrasings ("Order received", "Invoice
// arrives", "Application submitted") even with the prompt rules. This
// regex filters those out at the server-side guard below regardless of
// what the model produces — the Start node is rendered automatically;
// it must never be a numbered step.
const TRIGGER_PATTERNS = [
  // Direct passive-voice trigger forms
  /^(?:supplier|customer|client|user|employee|vendor|patient|member|applicant|contact)\s+(?:invoice|order|email|request|ticket|payment|enquiry|inquiry|application|form|message|booking|reservation|complaint|claim)\s+(?:received|comes? in|arrives?|lands?|drops?|hits?|is\s+(?:received|submitted|created|placed|opened|raised|logged|filed))/i,
  /^(?:invoice|order|email|request|ticket|payment|enquiry|inquiry|application|form|message|booking|reservation|complaint|claim|report|document|file)\s+(?:received|comes? in|arrives?|lands?|drops?|hits?|is\s+(?:received|submitted|created|placed|opened|raised|logged|filed|generated|delivered))/i,
  // Generic event verbs in passive voice as the whole step name
  /^(?:start|begin|process\s+(?:starts?|begins?|kicks?\s+off)|trigger(?:ed)?|kick[- ]?off)\b/i,
  // "<noun> submitted/triggers/initiated" patterns
  /^[\w\s]{1,40}\s+(?:submitted|initiated|triggered|kicks?\s+off|raised|opened|received|generated)$/i,
];
...
function sanitiseStepInput(name, input) {
  ...
  if (name === 'add_step' && (input.afterStep === 0 || input.afterStep == null) && looksLikeTrigger(input.name)) {
    // The agent tried to insert a trigger as the new Step 1. Convert
    // to a set_process_definition instead — drop the add_step entirely.
    return {
...
async function runStreamingLoop({ system, messages, onEmit, ctx, maxIterations = 10 }) {
  ...
  while (iterations < maxIterations) {
    ...
    // mistakes before they ever reach the canvas. The prompt tells
    // Reina not to do this; this catches the remaining cases where
    // the model relapses. Each tool-use produces (a) the sanitised
    // tool call (or null = drop) AND (b) any side-effect tool calls
    // injected by the sanitiser (e.g. set_process_definition for the
    // demoted trigger).
