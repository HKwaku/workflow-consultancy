...
function sanitiseStepInput(name, input) {
  ...
  if (name === 'replace_all_steps' && Array.isArray(input.steps) && input.steps.length > 0) {
    ...
    // Cap isMerge usage. The agent over-flags merges trying to be safe,
    // producing stacked teal circles. Two rules to filter the noise:
    //   1. No two consecutive isMerge steps — the second is always
    //      redundant since the first already pulls in branch terminals.
    //   2. After each decision, keep only the FIRST downstream isMerge
    ...
    //      rejoin nodes; the renderer needs exactly one.
    {
      ...
      stepsCopy = stepsCopy.map((s, i) => {
        ...
        // Strip a merge that immediately follows another merge.
        const prev = stepsCopy[i - 1];
        if (prev?.isMerge) return { ...s, isMerge: false };
        // Strip a merge if a previous merge has already been taken
