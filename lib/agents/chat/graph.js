...
async function runStreamingLoop({ systemPrompt, messages, onEmit }) {
  ...
  while (recursionCount < maxRecursion) {
    emit('progress', { message: recursionCount === 0 ? 'Reina is thinking…' : 'Updating your process map…' });
...
/** User-visible status while attachments are prepared for the model */
function describeAttachmentFile(a) {
  const t = (a.type || '').toLowerCase();
  const name = a.name || 'file';
  if (t.startsWith('image/')) return `image “${name}”`;
  if (t.includes('spreadsheet') || /application\/vnd\.ms-excel|spreadsheetml/.test(t) || /\.(xlsx?|csv)$/i.test(name)) {
...
export async function runChatAgent({ message, currentSteps, currentHandoffs, processName, history, incompleteInfo, attachments, editingReportId, editingRedesign, redesignContext, onProgress, onEmit }) {
  ...
  const systemPrompt = chatSystemPrompt({ processName, stepsDesc, incompleteBlock, editingMode, redesignContext });
  ...
  if (hasAttachments) {
    ...
    emit('progress', {
      message: `Received ${attachments.length} file${attachments.length > 1 ? 's' : ''}: ${list}. Preparing for analysis…`,
    });
  }
  ...
  if (hasRichContent) {
    ...
    for (const a of attachments) {
      ...
      if (IMAGE_TYPES.includes(a.type) && a.content) {
        ...
        // Trim large text files to avoid token limits (~80k chars ≈ ~20k tokens)
    ...
    if (hasAttachments) {
      emit('progress', { message: 'Packaging your files for the assistant…' });
    }
  ...
  // Emit an immediate acknowledgment so the user sees text right away before the LLM responds.
  let preAck = '';
  if (hasAttachments) {
    const fileDesc = attachments.length === 1
      ? describeAttachmentFile(attachments[0])
  ...
  const { textParts, actions } = await runStreamingLoop({
    systemPrompt,
    messages,
    onEmit,
    initialProgress: hasAttachments ? 'Analyzing your upload(s) and updating your map…' : undefined,
  });
