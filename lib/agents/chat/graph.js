...
async function executeTool(name, input, ctx) {
  switch (name) {
    ...
    case 'propose_add_system': {
      ...
      if (!name) return 'name is required.';
      if (name.length > 200) return 'name must be 200 characters or fewer.';
      const VALID_LAYER = new Set(['system_of_record', 'productivity', 'workflow', 'analytics', 'comms', 'other']);
    ...
    case 'propose_delete_system': {
      ...
      const SYS_LAYER = new Set(['system_of_record', 'productivity', 'workflow', 'analytics', 'comms', 'other']);
