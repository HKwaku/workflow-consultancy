 ...
 */
export function suggestedModelIdForPhase({ allowed, phase, hasAttachments }) {
  if (!Array.isArray(allowed) || allowed.length === 0) return null;
  const tier = pickTierForContext({ phase, hasAttachments });
...
function pickTierForContext({ phase, hasAttachments }) {
