...
- isMerge: true → convergence point where branches rejoin (any decision type — parallel, inclusive, or exclusive)
...
3. A parallel (parallel=true) or inclusive (inclusive=true) decision with no downstream isMerge step → branches never converge (parallel/inclusive always require a merge)
4. A step with branches but isDecision=false → gateway not rendered as diamond
5. isMerge=true on a step with NO decision upstream at all → orphaned merge. Flag and unset.
   For EXCLUSIVE decisions, isMerge is optional — set it only on the step where branches actually rejoin. If branches go their separate ways and never reconverge (e.g. "Rejected" ends the flow), don't set isMerge.
...
2. For each error-severity issue, call the appropriate repair tool:
   ...
   - Parallel/inclusive decision missing merge → mark_merge_node (pick the step after all branches complete)
   - Exclusive decision where branches clearly rejoin on a downstream step but isMerge is missing → mark_merge_node on the rejoin step
   - Orphaned isMerge (no decision precedes it) → flag_issue only
