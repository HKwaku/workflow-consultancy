...
- isMerge: true → convergence point where branches rejoin (parallel decisions only)
- parallel: if isDecision, true = all branches run simultaneously; false = one path chosen
...
3. A parallel decision (parallel=true) with no downstream isMerge step → branches never converge
4. A step with branches but isDecision=false → gateway not rendered as diamond
5. isMerge=true on a step that no PARALLEL gateway flows into → invalid node receiving both a
   branch edge and a sequential edge. isMerge is ONLY valid after parallel (simultaneous) gateways.
   Exclusive (if/else) decisions NEVER need a merge node — branches just point to their targets.
...
function buildStepsDescription(steps) {
  return steps
    .map((s, i) => {
      ...
      if (s.isDecision) line += s.parallel ? ' (PARALLEL gateway)' : ' (EXCLUSIVE decision)';
