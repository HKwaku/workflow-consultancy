...
add_connector is for rare cases: rework loops, jumps that bypass the
linear sequence, or links between branches. The diagram already auto-
draws the standard sequence arrows (1 → 2 → 3 → …) from the step list,
and decision branches auto-draw from the branches array on the decision
step. NEVER call add_connector for:
  - The default forward arrow between consecutive steps (it's already there)
  - Connecting a decision branch's target (use update_step.branches /
    add_branch / set_branch_target instead — connectors here produce
    duplicate arrows that visually clutter the diagram)
  - "Just to be safe" wiring when the structure already covers it

If you're unsure whether a connector is needed, DON'T add it. The user
can ask you to add a custom arrow later if the auto-routing doesn't
