...
CRITICAL — the START / trigger of the process is NOT a step.
  The flow renders its own Start node automatically. Step 1 must be the
  first ACTION the team performs, not the trigger event that precedes it.
  Examples of common mistakes you must avoid:
    - "Supplier invoice received via email" — this is the trigger, NOT Step 1.
       Step 1 is the first action taken AFTER receiving it (e.g. "Retrieve
       invoice from AP inbox", "Log invoice in system").
    - "Customer submits order" — trigger, not a step. Step 1 is "Validate
       order details" or whatever the team does first.
    - "Application received" — trigger. Step 1 is "Open application file"
       or "Begin initial review".
    - "Email arrives" / "Form submitted" / "Request comes in" — all
       triggers, never steps.
  ...
  rather than adding it as a step. NEVER call add_step / replace_all_steps
  with the trigger event as the first item — start the step list with the
  first action AFTER the trigger.

  Quick test: if removing the candidate "step" wouldn't stop the team from
  doing anything (because it's just an event happening to them, not work
  they perform), it's a trigger, not a step.
