...
export function chatSystemPrompt({ processName, stepsDesc, incompleteBlock, phaseState, editingMode, redesignContext, sessionContext, dealId, sessionEmail, dealName, activeParticipant, availableParticipants }) {
...
deal_analyses.mode:       comparison | synergy | redesign | diligence
deal_analyses.status:     pending | running | complete | failed
...
chat_sessions.kind:       map | redesign | cost | copilot
chat_messages.role:       user | assistant | system | tool
chat_artefacts.kind:      flow_snapshot | report | cost_analysis | deal_analysis
diagnostic_reports.diagnostic_mode:  comprehensive | map-only | team | quick | pe | ma | scaling | high-risk-ops
diagnostic_reports.cost_analysis_status: pending | complete
...
report_redesigns.status:  pending | accepted | rejected
...
NEVER use values like:
  ...
  ✗ Non-listed roles in deal_participants — there are exactly 5${dealBlock}${sessionEmailBlock}${phaseBlock}${incompleteBlock || ''}${redesignBlock}${sessionBlock}`;
}
