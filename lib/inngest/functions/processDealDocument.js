...
import { maybeAutoTriggerDealAnalysis } from '@/lib/deal-analysis/autoTrigger';
...
export const processDealDocument = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // 8. Auto-trigger a delta diligence run. Only fires when there's a
    //    prior completed analysis on this deal AND the throttle window has
    //    passed AND no analysis is currently in flight (see autoTrigger.js).
    //    Best-effort — never throws.
    await step.run('maybe-auto-trigger-analysis', async () => {
      const result = await maybeAutoTriggerDealAnalysis({ sb, dealId: deal_id });
      return result;
    });
