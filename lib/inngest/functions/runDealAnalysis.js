...
import { getChatModel, getFastModel } from '@/lib/agents/models';
...
export const runDealAnalysis = inngest.createFunction(
  {
    ...
    // Inngest free tier caps function concurrency at 5. Bump back to a
    // higher value once on a paid plan; for now 5 is the practical
    // ceiling and matches the plan default.
    concurrency: { limit: 5 },
  },
  ...
  async ({ event, step }) => {
    ...
    const llmResult = await step.run('llm-call', async () => {
      ...
      // Vercel Hobby caps the function timeout at 60s. Sonnet 4.6 takes
      // 90-150s for a full redesign JSON response — too slow for Hobby.
      // Use Haiku for the LLM step here; it outputs ~3-5x faster (25-40s)
      // and produces structurally-correct redesign JSON in the same shape.
      // Trade-off: phasing/risk narratives are slightly less nuanced than
      // Sonnet's, but the redesigned-process steps + change-overview are
      // identical quality. On Vercel Pro, switch back to getChatModel
      // (Sonnet) and raise maxTokens to 8192 for the higher quality.
      const model = getFastModel({ maxTokens: 6144, temperature: 0, apiKey: api_key });
      try {
