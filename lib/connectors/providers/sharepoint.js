...
registerProvider({
  ...
  async pickFolder({ accessToken, query }) {
    ...
    if (query?.kind === 'sites') {
      ...
      // OneDrive fallback. If both site-level calls failed (e.g. the
      // OAuth token is from a tenant context that doesn't publish
      // SharePoint to the app, even if the user technically has an SPO
      // licence elsewhere — a common Graph quirk on /common-issued
      // tokens), or even if they succeeded, also offer the user's
      // personal drive(s). /me/drives works for personal MS accounts
      // AND business accounts without SPO; the picker can then bind a
      // OneDrive folder instead of a SharePoint folder.
      try {
        ...
        if (drivesResp.ok) {
          ...
          for (const d of (j.value || [])) {
            ...
            // step ('drives' kind) ignores site_id when it sees the
            // synthetic onedrive id and lists this drive directly.
            out.push({
              id: `onedrive:${d.id}`,
      ...
      } catch { /* tolerate, will throw below if everything failed */ }
      if (out.length === 0) {
        // Nothing worked — surface whichever error was most informative
        // so the user can act (most often this is a permissions / consent
        // issue on the Entra app, or a tenant without SPO + without
        // OneDrive). Include the SPO-license message verbatim because it
