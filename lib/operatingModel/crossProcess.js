...
export function computeFunctionHeatmap({ reports, findings, processSystems, functions }) {
  ...
  for (const r of reports || []) {
    ...
    for (const proc of procs) {
      ...
      const procContribByCap = new Map(); // capId -> { workMinutes, savings, systems }
      const tally = (capId) => {
        if (!procContribByCap.has(capId)) procContribByCap.set(capId, { workMinutes: 0, savings: 0, systems: new Set() });
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (wmValid > 0 && totalWm > 0 && annualCost > 0) {
          const stepCost   = (wmValid / totalWm) * annualCost;
          const rate       = autoRateFor(step, i, proc);
          const stepSaving = stepCost * rate;
          if (stepSaving > 0) {
            b.derivedSavings += stepSaving;
            tally(stepCapId).savings += stepSaving;
          }
      ...
      for (const [capId, contrib] of procContribByCap.entries()) {
        ...
        entry.savings     += contrib.savings;
        entry.automatableShare = procAutomatableShare;
        // Cost share = this function's slice of the process's annual cost,
        // proportional to the work minutes it did.
        if (totalWm > 0 && annualCost > 0 && contrib.workMinutes > 0) {
          entry.annualCost += (contrib.workMinutes / totalWm) * annualCost;
        }
