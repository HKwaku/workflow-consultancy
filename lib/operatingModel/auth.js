...
export async function resolveDefaultModelForUser({ email, userId }) {
  ...
  try {
    ...
    // Members → org → default model in one round-trip via embed.
    const headers = getSupabaseHeaders(sb.key);
    ...
    const modelId = row.organization?.default_operating_model_id;
    if (!modelId) return { modelId: null, organizationId: orgId, reason: 'no_default_model' };
    return { modelId, organizationId: orgId, isAdmin: !!row.is_org_admin };
  } catch (e) {
