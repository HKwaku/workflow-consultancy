...
**Workspace UX**
- `/workspace` hosts `DiagnosticClient` (the same shell `/process-mapping` uses) and auto-opens the workspace overlay scoped to the user's **active operating model** (per-member preferred → org default; see [Operating models — multiple per org](#operating-models--multiple-per-org)). The graph view is the default tab.
...
| `/process-mapping` | Run / resume process mapping (the chat-driven canvas). Canonical alias → `/workspace/map`. The old `/process-audit` path still redirects here so external links / n8n / Calendly / bookmarks don't 404. | Optional (required for comprehensive + team) |
...
- Autosave: every edit upserts `processes.flow_data` (no manual "save"; the old per-step `?resume=ID&step=N` resume URL is gone with `/api/progress`)
- Drag-and-drop file upload (Excel, DOCX, PDF, PPTX, images) for process extraction
- **Share with a colleague**: `ShareProcessLink` (canvas back-bar + workspace process list) copies `/process-mapping?view=<id>`. The `get-diagnostic` GET read-only branch is public by design (UUID is the unguessable bearer); the editable path still gates on owner / RLS, so the link is read-only and never grants edit. Scoped to non-deal flows (deal sharing stays on deal collaborators). Replaces the old handover modal + `/api/progress` resume link.
...
There is no per-type page component anymore — all three render through the chat workspace at `/process-mapping?deal=<id>` with `DealWorkspaceModal` as the surface. The legacy per-type pages (`DealPagePE.jsx` / `DealPageMA.jsx` / `DealPageScaling.jsx`) and `DealAnalysisSection.jsx` were removed in Phase 18; `app/deals/[id]/page.jsx` is now a redirect.
...
4. Each participant fills a `deal_flow` slot at `/process-mapping?dealFlowId=…` — when they save the diagnostic, `deal_flows.report_id` is populated
...
- **Switcher UI (two surfaces):**
  - `components/diagnostic/chat/WorkspaceContextStrip.jsx` — the **primary, Home-reachable** switcher. It's the "Working in **&lt;Model&gt; ▾**" banner at the top of the chat surface (where `HomeRailButton` lands). The model name is a dropdown: org models lazily loaded from `/api/me/operating-models` (ACTIVE + default marked) plus **"+ New model"**. Select → `PUT` then hard-nav to `/workspace/map` (whole shell re-resolves); create → prompt → `POST` (auto-activates) → fresh model. This is what makes "start a new workspace context from Home" possible.
  - `components/workspace/WorkspaceModelsTab.jsx` — the Standard-scope model picker in `DiagnosticWorkspace` (same list/badges/"+ New model"); a click persists via `PUT` so it survives Home / New chat.
...
| File | Purpose |
|------|---------|
| `app/process-mapping/page.jsx` | Route entry |
