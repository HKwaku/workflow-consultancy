...
export default function SignInForm({ supabase, onAuthenticated, mode: initialMode }) {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const redirectUrl = window.location.origin + '/workspace/map';
  ...
  return (
    <div className="auth-card">
      ...
      <p className={mode === 'login' ? 'auth-subtitle auth-subtitle--signin' : 'auth-subtitle'}>
        {mode === 'signup'
          ? 'Sign up to open your operating-model workspace.'
          : 'Sign in to your operating-model workspace.'}
      </p>
      {error && <div className="auth-error show">{error}</div>}
      {success && <div className="auth-success show">{success}</div>}
      {hydrated && !supabase && (
